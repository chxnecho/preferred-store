  ...
  <header class="header">
    <div class="container header-inner">
      ...
      <nav class="nav">
        ...
        <router-link to="/orders" v-if="auth.isLoggedIn()">我的订单</router-link>
        <button class="cart-btn" @click="$router.push('/cart')">
          🛒 购物车
          <span class="cart-count" v-if="cart.totalQty > 0">{{ cart.totalQty }}</span>
        </button>
        <template v-if="auth.isLoggedIn()">
          <div class="user-menu" @click.stop>
            ...
            <div class="dropdown" v-if="menuOpen" @click="menuOpen = false">
