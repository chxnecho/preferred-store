  ...
  <main class="container products-page">
    ...
    <div class="toolbar">
      ...
      <span v-if="keyword" class="search-hint">
        “{{ keyword }}” 的搜索结果
        <a @click.prevent="clearKeyword" href="#">清除</a>
      </span>
    </div>
    ...
    <div class="pagination" v-if="totalPages > 1">
      ...
      <button :disabled="page >= totalPages" @click="setFilter({ page: page + 1 })">下一页 ›</button>
    </div>
  </main>
</template>
...
api.categories().then((d) => (categories.value = d.categories)).catch(() => {});
</script>
...
.filter-btn {
  ...
  padding: 8px 18px; font-size: 14px; transition: all 0.2s;
}
.filter-btn small { color: var(--text-light); font-size: 12px; }
.filter-btn.active, .filter-btn:hover { background: var(--primary); border-color: var(--primary); color: #fff; }
.filter-btn.active small, .filter-btn:hover small { color: rgba(255,255,255,0.85); }

.toolbar { display: flex; align-items: center; gap: 16px; margin: 16px 0 20px; flex-wrap: wrap; }
.sort-group { display: flex; gap: 4px; background: #fff; border-radius: 999px; padding: 4px; }
.sort-group button { background: none; padding: 7px 14px; border-radius: 999px; font-size: 13px; color: var(--text-light); }
.sort-group button.active { background: var(--primary-light); color: var(--primary); font-weight: 600; }
.search-hint { font-size: 13px; color: var(--text-light); }
.search-hint a { color: var(--primary); margin-left: 6px; }
