    ...
    <template v-else>
      ...
      <section class="status-card" :class="order.status">
        ...
        <div class="status-actions">
          <button v-if="order.status === 'pending'" class="btn-primary" @click="pay">
            立即支付（模拟）
          </button>
          <button v-if="order.status === 'pending'" class="btn-outline" @click="cancel">
            取消订单
          </button>
          <button v-if="order.status === 'paid'" class="btn-primary" @click="confirmReceipt">
            确认收货
          </button>
        </div>
      </section>
