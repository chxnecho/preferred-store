...
export default function CheckoutView() {
  ...
  const canSubmit = selectedAddressId !== null && totalQty > 0
  ...
  async function saveAddress(formData: AddressInput) {
    setAddrError("")
    setAddrSaving(true)
    try {
      ...
      setAddrError(err instanceof Error ? err.message : "保存失败")
    } finally {
      setAddrSaving(false)
    }
  }
  ...
  useEffect(() => {
    Promise.all([cart.fetchCart(), loadAddresses()])
      .catch((err: unknown) => toast(err instanceof Error ? err.message : "加载失败", "error"))
