      ...
      <div class="detail-info">
        ...
        <div v-if="product.stock > 0" class="qty-row">
          ...
          <div class="qty-control">
            <button :disabled="qty <= 1" @click="qty = Math.max(1, qty - 1)">−</button>
        ...
        <div class="detail-actions">
          <template v-if="product.stock > 0">
            <button class="btn-outline" :disabled="submitting" @click="addToCart(false)">
              加入购物车
            </button>
            <button class="btn-primary buy-btn" :disabled="submitting" @click="addToCart(true)">
              {{ submitting ? "处理中..." : "立即购买" }}
            </button>
          </template>
        ...
        <div class="service-line">🚚 极速配送 · ✅ 正品保障 · 🔄 七天无理由退换</div>
      </div>
    </div>
  </main>
</template>
...
watch(
  () => route.params.id,
  () => route.name === "product-detail" && load()
)
</script>
