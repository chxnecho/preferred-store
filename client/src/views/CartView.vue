    ...
    <template v-else>
      <div class="cart-list">
        <div v-for="item in cart.items" :key="item.productId" class="cart-row">
          ...
          />
          <div class="row-img" :style="{ background: item.bg }" @click="$router.push(`/product/${item.productId}`)">
            {{ item.emoji }}
          </div>
          <div class="row-info">
          ...
          <div class="qty-control">
            <button @click="changeQty(item.productId, item.qty - 1)" :disabled="busy">−</button>
