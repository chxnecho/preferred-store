...
// 用法：node server/seed.js [--force]   --force 会清空商品表后重灌
...
if (count > 0 && !force) {
  ...
  if (force) {
    db.exec(
      "DELETE FROM order_items; DELETE FROM orders; DELETE FROM cart_items; DELETE FROM products;"
    )
...
if (!db.prepare("SELECT id FROM users WHERE username = ?").get("demo")) {
  ...
  const uid = db.prepare("SELECT id FROM users WHERE username = ?").get("demo").id
  db.prepare(
