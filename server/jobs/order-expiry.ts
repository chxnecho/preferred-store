...
import { db, dbAll, withTransaction } from "../db"

const CHECK_INTERVAL_MS = 60 * 1000

/** 取消所有已过期待支付订单，返回取消数量 */
export function cancelExpiredOrders(): number {
  try {
    const cancelled = withTransaction<number>(() => {
      const expired = dbAll<{ id: number }>(
        db.prepare(
      ...
      const cancelStmt = db.prepare("UPDATE orders SET status = 'cancelled' WHERE id = ? AND status = 'pending'")
      for (const { id } of expired) {
        for (const it of dbAll<{ product_id: number; qty: number }>(itemsStmt, id)) {
...
/** 启动定时清理任务（启动时先执行一次） */
export function startOrderExpiryJob(): ReturnType<typeof setInterval> {
  cancelExpiredOrders()
