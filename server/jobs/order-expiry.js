...
export function cancelExpiredOrders() {
  try {
    const cancelled = withTransaction(() => {
      ...
      const restoreStmt = db.prepare("UPDATE products SET stock = stock + ?, sales = MAX(sales - ?, 0) WHERE id = ?");
    ...
    if (cancelled > 0) console.log(`[order-expiry] 已自动取消 ${cancelled} 个超时未支付订单并回补库存`);
    return cancelled;
  } catch (err) {
