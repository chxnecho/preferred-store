...
router.get("/", (req, res) => {
  ...
  const list = db
    .prepare(
      `SELECT id, name, description, price, sales, category, emoji, bg, stock
...
router.get("/categories", (_req, res) => {
  const rows = db
    .prepare("SELECT category AS name, COUNT(*) AS count FROM products GROUP BY category ORDER BY COUNT(*) DESC")
    .all();
