...
function shape(a) {
  return {
    ...
    region: a.region, detail: a.detail, isDefault: !!a.is_default,
  };
}
...
router.post("/", (req, res) => {
  ...
  const id = withTransaction(() => {
    ...
    const info = db
      .prepare("INSERT INTO addresses (user_id, receiver, phone, region, detail, is_default) VALUES (?, ?, ?, ?, ?, ?)")
      .run(req.user.id, String(receiver).slice(0, 30), String(phone), String(region).slice(0, 60), String(detail).slice(0, 120), makeDefault ? 1 : 0);
    return info.lastInsertRowid;
  });
  res.status(201).json({ address: shape(db.prepare("SELECT * FROM addresses WHERE id = ?").get(id)) });
});
...
router.delete("/:id", (req, res) => {
  const info = db.prepare("DELETE FROM addresses WHERE id = ? AND user_id = ?").run(req.params.id, req.user.id);
