...
import type { CartItem, CartPayload } from "../../shared/types"

const router = Router()
router.use(authRequired)

function getCartItems(userId: number): CartItem[] {
...
function getCartPayload(userId: number): CartPayload {
...
router.post("/", (req, res) => {
  ...
  const row = dbGet<{ qty: number }>(
    db.prepare("SELECT qty FROM cart_items WHERE user_id = ? AND product_id = ?"),
    req.user.id,
    productId
  )
...
router.put("/:productId", (req, res) => {
  ...
  if (qty <= 0) {
    ...
    const stock =
      dbGet<{ stock: number }>(db.prepare("SELECT stock FROM products WHERE id = ?"), productId)
        ?.stock ?? 0
    if (stock <= 0) {
