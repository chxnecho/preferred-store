...
const { cancelExpiredOrders } = await import("../jobs/order-expiry")
...
// ---- 测试数据 ----
let userAlice = 0 // 用户 id
let pStock = 0 // 正常商品：库存 10，单价 19.9
let pSoldOut = 0 // 售罄商品：库存 0
let pTight = 0 // 小库存商品：库存 3（用于库存不足回滚）
let addrId = 0
...
before(() => {
  ...
  pStock = Number(insProduct.run("测试商品A", "desc", 1990, "测试", 10).lastInsertRowid)
  pSoldOut = Number(insProduct.run("测试商品B", "desc", 500, "测试", 0).lastInsertRowid)
  pTight = Number(insProduct.run("测试商品C", "desc", 1000, "测试", 3).lastInsertRowid)
...
async function login(
  ...
  const { status, data } = await req<{ token?: string }>("/api/auth/login", {
    method: "POST",
...
describe("products", () => {
  ...
  it("已下架商品从列表与详情中隐藏", async () => {
    ...
    assert.ok(!list.data.list.some((p: any) => p.id === pSoldOut))
...
describe("cart", () => {
  it("加入购物车数量被库存截断", async () => {
    ...
    const item = r.data.items.find((i: any) => i.productId === pStock)
  ...
  it("修改数量超过库存时被截断，qty<=0 删除条目", async () => {
    ...
    assert.equal(put.data.items.find((i: any) => i.productId === pStock).qty, 10)

    const del = await req(`/api/cart/${pStock}`, { method: "PUT", token, body: { qty: 0 } })
    assert.equal(del.status, 200)
    assert.ok(!del.data.items.some((i: any) => i.productId === pStock))
  })
})
...
describe("orders", () => {
  ...
  it("下单成功：扣库存、加销量、写快照、设过期时间、清购物车", async () => {
    ...
    assert.ok(!cart.data.items.some((i: any) => i.productId === pStock))
  })
  ...
  it("取消待支付订单回补库存与销量，已完成订单不可取消", async () => {
    ...
    const completed = list.data.list.find((o: any) => o.status === "completed")
  ...
  it("订单列表返回商品明细（批量查询）", async () => {
    ...
    for (const o of list.data.list as any[]) assert.ok(Array.isArray(o.items) && o.items.length > 0)
  })
})
...
describe("addresses", () => {
  it("取消唯一默认地址后仍保证存在默认地址", async () => {
    ...
    const def = list.data.list.find((a: any) => a.isDefault)
